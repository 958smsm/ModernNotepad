...
The grammar-mode switch offers five choices: **Logic & Traditional NLP**, **OpenAI**, **Python spaCy**, **Python NLTK**, and **Google Cloud Natural Language**. Traditional mode works without a network connection, Python, or an API key. Its embedded, generated WordNet/Brown lexical database contains **108,845 words**, including frequency-weighted alternative categories for **11,314 ambiguous words**. Productive morphology covers regular inflections and compounds while explicit exception data covers irregular verbs, plurals, and comparisons.

The Traditional analyzer is sentence/clause-aware and uses contextual disambiguation for noun/verb/adjective ambiguity, unknown words, proper names and acronyms, subject/object roles, relative/content clauses, inversion, coordination, gerunds/participles, contractions, and function words. Articles/non-quantifying determiners have a dedicated **Determiner** category, while infinitival and recognized phrasal-verb particles have a dedicated **Particle** category; true amount/scope words remain **Quantifier**. Large-document analysis keeps complete core results while bounding only the number of WPF visual overlays. See [Testing](docs/TESTING.md) for the reproducible Universal Dependencies English EWT accuracy benchmark.
