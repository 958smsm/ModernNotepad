...
public sealed class GrammarColorAnalyzer
{
    ...
    {
        "break down", "bring up", "call off", "carry out", "come back", "cut off", "figure out", "fill out",
        "find out", "give up", "go on", "grow up", "hold on", "keep on", "leave out", "log in", "look up",
        "pick up", "point out", "put off", "put on", "set up", "shut down", "sign in", "take off", "take over",
        "turn down", "turn off", "turn on", "turn up", "work out", "write down"
    };
    ...
    private static readonly HashSet<string> CommonNouns = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "planning", "setting", "meaning", "marketing", "engineering", "coding", "programming", "learning", "drawing", "painting", "feeling",
        ...
        bool isQuestion)
    {
        ...
        if (IsContractedPronounAuxiliary(word))
        {
            return LexicalKind.Pronoun;
        }

        if (IsContractedAuxiliaryVerb(word))
        {
        ...
        if (InterrogativeWords.Contains(word))
        {
            ...
            if (isQuestion || IsEmbeddedQuestion(tokens, range, local))
            {
        ...
        if (SubordinatingConjunctions.Contains(word) && !IsPrepositionalUse(tokens, range, local))
        {
            return LexicalKind.Conjunction;
        }
        ...
        bool isQuestion)
    {
        ...
        if (HasFollowingNominal(tokens, range, local))
        {
            ...
            if (local > 0 && IsVerbWord(previous)
                && (Determiners.Contains(next) || PossessiveDeterminers.Contains(next) || PersonalPronouns.Contains(next)))
            {
        ...
        int local)
    {
        ...
        if (LexiconCategory(word) != GrammarCategory.Adjective)
        {
            return false;
        }
    ...
    private static bool LooksLikeVerb(IReadOnlyList<TextToken> tokens, SpanTokenIndex.TokenRange range, int local)
    {
        ...
        var lexicon = LexiconCategory(word);
        if (lexicon == GrammarCategory.Verb)
        {
            score += 6;
        }
        else if (lexicon is GrammarCategory.ObjectNoun or GrammarCategory.SubjectNoun)
        {
            score -= 2;
        }
    ...
    private static bool ShallowFiniteVerbCandidate(IReadOnlyList<TextToken> tokens, SpanTokenIndex.TokenRange range, int local)
    {
        ...
        if (ModalVerbs.Contains(word) || AuxiliaryVerbs.Contains(word) || CommonVerbs.Contains(word)
            || LexiconCategory(word) == GrammarCategory.Verb)
        {
            return true;
        }
        ...
        {
            var next = tokens[range.Start + local + 1].Normalized;
            return Determiners.Contains(next) || PossessiveDeterminers.Contains(next) || PersonalPronouns.Contains(next);
        }
        ...
        IReadOnlyList<LexicalKind> kinds)
    {
        ...
        for (var local = 1; local < range.Count; local++)
        {
            ...
            var commaBoundary = hasComma && (hasVerb || markerBoundary || startsWithPreposition);
    ...
    private static GrammarCategory ClassifyStandalone(TextToken token)
    {
        ...
        if (IsContractedPronounAuxiliary(word)) return GrammarCategory.Pronoun;
        if (IsContractedAuxiliaryVerb(word)) return GrammarCategory.Verb;
        ...
        int local)
    {
        ...
        var previous = tokens[range.Start + local - 1].Normalized;
        return PhrasalVerbParticlePairs.Contains($"{previous} {word}");
    }
    ...
    private static bool LooksLikeInfinitive(IReadOnlyList<TextToken> tokens, SpanTokenIndex.TokenRange range, int local)
    {
        ...
        if (ModalVerbs.Contains(next) || AuxiliaryVerbs.Contains(next) || CommonVerbs.Contains(next)
            ...
            || LexiconCategory(next) == GrammarCategory.Verb)
        {
            return true;
        }

        return next.Length > 3 && (next.EndsWith("ize", StringComparison.OrdinalIgnoreCase)
    ...
    private static bool IsPrepositionalUse(IReadOnlyList<TextToken> tokens, SpanTokenIndex.TokenRange range, int local)
    {
        var word = tokens[range.Start + local].Normalized;
        if (!Prepositions.Contains(word))
        {
        ...
        return !HasFollowingFiniteVerb(tokens, range, local, 4);
    }
    ...
    private static bool RawLooksNominal(string word)
    {
        ...
        var category = LexiconCategory(word);
        if (category is GrammarCategory.ObjectNoun or GrammarCategory.SubjectNoun)
        {
            return true;
        }
    ...
    private static bool IsVerbWord(string word)
    {
        ...
        return LexiconCategory(word) == GrammarCategory.Verb;
    }

    private static bool IsAdverbByLexiconOrMorphology(string word)
    {
        if (LexiconCategory(word) == GrammarCategory.Adverb)
        {
            return true;
        }
    ...
    private static string NormalizeApostrophe(string word)
        => word.IndexOf('’') >= 0 ? word.Replace('’', '\'') : word;
    ...
    private static GrammarCategory? LexiconCategory(string word) =>
        GrammarLexicon.Lexicon.TryGetValue(word, out var category) ? category : null;
